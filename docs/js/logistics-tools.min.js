let scMain = (source, dest) => {
  ...
  result.consignments = [];
  $.getJSON("/trunkcontainers/" + id + "/events").done(events => {
    let cons = [];
    $.each(events, (_i, e) => {
    ...
    if (records.length !== 0) {
      $.each(records, (_i, record) => {
        $.post("/trunkcontainers/" + dest + "/scan/" + record).done(response => {
          let r = {
            barcode: record,
...
function addPartsToDOM() {
  ...
  $("div.page-content > div > div").empty().append(ltContainer);
  ...
  $("#lt_insert").append(ciForm);
  $.each(ciStatus, (_i, v) => $("<option>", {
    value: v,
    text: v
  }).appendTo($("#ci_status")));
  $("#ci_btn").click(() => {
    let fields = [ "tracking_number", "requested_route", "consolidation_id", "location", "delivery_address_type", "package_type", "status" ];
    let data = {
      q: "collected:SW",
      count: 1e3,
      client_id: 11270,
  ...
  let scForm = $("<div>", {
    ...
    id: "sc_old"
  }).attr(scValid)).append($("<input>", {
    id: "sc_new"
  }).attr(scValid)).append($("<button>", {
    id: "sc_btn"
  }).text("Move Records"));
  $("#lt_insert").append(scForm);
  $("#sc_btn").click(() => {
